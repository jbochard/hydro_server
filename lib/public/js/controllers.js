...
ang.controller('HydroponicController', function($scope, $location, nurseriesService, plantsService, plant_typesService, mesurementsService) {

  $scope.template = '';
  $scope.nurseries = [];
  $scope.plants = [];

  $scope.nursery_types = [{ name: "Cajón", type: "drawer"}, { name: "Almácigos", type: "nursery"}, { name: "Maceta", type: "plantpot"}];
  $scope.new_nursery = {};
  ...
  $scope.sel_mesurement= { type: null, value: 0 };
  $scope.mesurements = []
  ...
  function refreshMesurements() {
      ...
      function(result) {
        ...
        $scope.sel_mesurement.type = $scope.mesurements[0].type;
      },
      function(result) {
  ...
  $scope.register_mesurement = function() {
    nurseriesService.register_mesurement($scope.sel_nursery._id, $scope.sel_mesurement.type, $scope.sel_mesurement.value, 
      function(result) {
      },
      function(result) {
      });
  ...
  $scope.register_tallness = function() {
