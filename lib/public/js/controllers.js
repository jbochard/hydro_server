  ...
  $scope.show_nursery = function(id) {
    nurseriesService.get(id, function(response) {
      ...
      if ($scope.sel_nursery.hasOwnProperty('water_events') && $scope.sel_nursery.water_events.length > 0) {
        var timeDiff = Math.abs(new Date().getTime() - new Date($scope.sel_nursery.water_events[$scope.sel_nursery.water_events.length - 1].date).getTime());
        $scope.sel_nursery_max_water_change =  Math.floor(timeDiff / (1000 * 3600 * 24));
      } else {
