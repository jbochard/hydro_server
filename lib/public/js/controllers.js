...
ang.controller('HydroponicController', function($scope, $location, nurseriesService, plantsService, plant_typesService, mesurementsService) {
  ...
  $scope.show_nursery = function(id) {
    nurseriesService.get(id, function(response) {
      ...
      if ($scope.sel_nursery.hasOwnProperty('water_events') && $scope.sel_nursery.water_events.length > 0) {
        $scope.sel_nursery_max_water_change = $scope.sel_nursery.water_events[$scope.sel_nursery.water_events.length - 1].date;
      } else {
        $scope.sel_nursery_max_water_change = "";
      }
