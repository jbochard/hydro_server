...
ang.controller('HydroponicController', function($scope, $location, nurseriesService, plantsService) {
  ...
  $scope.set_plant_in_bucket = function(plant_id) {
    nurseriesService.patch_set_plant_in_bucket($scope.sel_nursery._id.$oid, $scope.sel_bucket_pos, plant_id.$oid, 
      function(result) {
  ...
  $scope.remove_plant_from_bucket = function(plant_id) {
    plantsService.patch_remove_plant_from_bucket(plant_id.$oid, 
      function(result) {
        $scope.show_nursery($scope.sel_nursery._id);
        refreshPlants();
      },
  ...
  $scope.add_plant = function() {
    $scope.plant_selected = { code: 0, creation_date: new Date(), type: "Albahaca" };
    $scope.template = '/plant_add.html'; 
  }

    $scope.remove_plant = function(plant_id) {
      plantsService.delete(plant_id.$oid,
        function(result) {
          refreshPlants();
          $scope.show_nursery($scope.sel_nursery._id);
        }, 
  ...
  $scope.show_plant = function(plant_id) {
    plantsService.get(plant_id.$oid,
        function(result) {
  ...
  $scope.update_plant = function() {
    plantsService.post(plant_selected,
      function(result) {
        refreshPlants();
        $scope.show_nursery($scope.sel_nursery._id);
      },
