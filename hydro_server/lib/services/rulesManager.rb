...
class RulesManager
	...
	def create(name, condition, action)
		id = BSON::ObjectId.new.to_s
		rule = Rule.new(name, condition, action, false, @sensorService)
		@rules[id] = rule
		@mongo_client[:rules].insert_one({ :_id => id, :name => name, :active => false, :condtion => condition, :action => action })
		id
	end
